...
def compute_likelihood(cellDensity, cellSpeed, cellNumber, densityMea, speedMea, densityMeaMean, densityMeaStd,
                       speedMeaMean, speedMeaStd):
    ...
    for i in range(0, int(sum(H))):
        if i < sum(densityMea != 0):
            modelLikelihood = modelLikelihood * 1.0 / (densityMeaStd * sqrt(2 * pi)) * exp(
                -(diff[i] - densityMeaMean) * (diff[i] - densityMeaMean) / (2 * densityMeaStd * densityMeaStd))
        else:
            modelLikelihood = modelLikelihood * 1.0 / (speedMeaStd * sqrt(2 * pi)) * exp(
                -(diff[i] - speedMeaMean) * (diff[i] - speedMeaMean) / (2 * speedMeaStd * speedMeaStd))
    return modelLikelihood


def compute_particle_likelihood(cellDensity, cellSpeed, cellNumber, sample, densityMea, speedMea, densityMeaMean,
                                densityMeaStd, speedMeaMean, speedMeaStd):
